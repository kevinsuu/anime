...
Run: `grep -n "backend:" -A 30 <repo>/docker-compose.yml | head -40`
...
Run: `cat <repo>/deploy/docker-compose.yml`
...
Run: `docker compose -f <repo>/deploy/docker-compose.yml config --quiet && echo "YAML OK"`
...
Run: `grep -n "imgDebug\|dbgSnapshot\|dumpBlanks\|__cardDebug\|__dumpCards" <repo>/frontend/app/components/AnimeGridCard.vue`
...
Run: `grep -n "data-imgph" <repo>/frontend/app/components/AnimeGridCard.vue`
...
Run: `grep -n "imgDebug\|imgdebug\|__cardDebug\|__dumpCards" <repo>/frontend/app/components/AnimeGridCard.vue`
Expected: 無輸出（完全清除)
